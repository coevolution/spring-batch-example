 ...
 * @Version 1.0
 */
//@Import(SampleBatchConfigurer.class)
@Configuration public class SimpleJobConfiguration {
    ...
    @Bean public Job premium_schedule_job(JobRepository jobRepository,Step step1) throws Exception {
        return this.jobBuilderFactory.get("premium_schedule_job").repository(jobRepository).start(step1).next(step2()).build();
    }

    @Bean public Step step1(PlatformTransactionManager transactionManager, ItemReader<SinglePremiumScheduleDTO> reader,
        ItemWriter<SinglePremiumScheduleDTO> writer) throws Exception {
        return this.stepBuilderFactory.get("step1").transactionManager(transactionManager).<SinglePremiumScheduleDTO,SinglePremiumScheduleDTO>chunk(1).reader(reader).writer(writer)
            .build();
    }
    ...
    @Bean
    public JdbcPagingItemReader<SinglePremiumScheduleDTO> reader(DataSource dataSource) throws Exception {
        ...
        queryProvider.setSelectClause("select id,global_Platform_Id,loan_Order_Id,user_Id,amount,user_Due_At,cleared,status");
        queryProvider.setFromClause("from loandb.qs_single_premium_schedule");
        queryProvider.setWhereClause("where user_Due_At between :userDueAtStart and :userDueAtEnd and cleared = false");
    ...
    @Bean public JdbcBatchItemWriter<SinglePremiumScheduleDTO> writer(DataSource dataSource) {
        String batchNo = DateUtil
            .formatDate(new Date(), "yyyyMMddHHmmss");
        return new JdbcBatchItemWriterBuilder<SinglePremiumScheduleDTO>().dataSource(dataSource).sql(SQL_UPDATE).itemPreparedStatementSetter((item,preparedStatement)->{
